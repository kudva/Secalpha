...
def sentitest(search_term, num_tweets):
...
def store(tweets):
    ...
    mongo_client = pymongo.MongoClient('23.246.199.110', 10025)
...
def print_results():
    ...
    for result in mean_results:
       if result['_id'] == 'negative':
          avg_neg_score = result['avgScore'] 
       if result['_id'] == 'positive':
          avg_pos_score = result['avgScore'] 
